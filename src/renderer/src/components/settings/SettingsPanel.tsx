...
export function SettingsPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative z-10 flex h-full w-[400px] max-w-[90vw] flex-col border-l border-[#2a2a2a] bg-[#0c0c0c]">
        <div className="flex items-center justify-between border-b border-[#2a2a2a] px-4 py-3">
          <span className="text-xs text-[#999]">settings</span>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex gap-1 border-b border-[#2a2a2a] px-4 py-2">
          <button
            ...
            className={`rounded px-2.5 py-1 text-[11px] transition ${
              activeTab === 'models' ? 'bg-[#1e1e1e] text-[#ccc]' : 'text-[#666] hover:text-[#999]'
            }`}
          >
            models
          </button>
                    ...
                    {configured.map((p) => (
                      <div key={p.id}>
                        <button
                          ...
                          className="flex w-full items-center justify-between rounded border border-[#2a2a2a] bg-[#161616] px-3 py-2.5 text-left text-xs transition hover:border-[#3a3a3a]"
                        >
                    ...
                    {unconfigured.map((p) => (
                      <div key={p.id}>
                        <button
                          type="button"
                          onClick={() => setExpandedId(expandedId === p.id ? null : p.id)}
                          className="flex w-full items-center justify-between rounded border border-dashed border-[#2a2a2a] px-3 py-2.5 text-left text-xs transition hover:border-[#3a3a3a]"
                        >
              ...
              <section>
                {!addingCustom ? (
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-1.5 rounded border border-dashed border-[#2a2a2a] px-3 py-2.5 text-xs text-[#666] transition hover:border-[#3a3a3a] hover:text-[#999]"
                  >
                    + custom provider
                  </button>
                ) : (
